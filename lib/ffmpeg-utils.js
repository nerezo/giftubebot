...
var pathToFfprobe = require('ffprobe-static').path;
...
var cropVideoFromUrl = function(start, to, resolution, realUrl, videoId) {
  ...
  // Check the passed resolution value and decide scale ratio. This scale rule reduce the resolution fifty percent.
  var scale = ' ';
  if (resolution) {
    var horizontalRes = Number(resolution.res.split('x')[0]);
    scale = (horizontalRes < 640 ? ' ' : ' -vf "scale=trunc(iw/4)*2:trunc(ih/4)*2" ');
  }
  ...
  if (realUrl.indexOf('m3u8') > -1) {
    ffmpegCommand = '"' + pathToFfmpeg + '" -loglevel ' + ffmppegLogLevel + ' -i "' + realUrl + '" -c copy -ss ' + start + ' -t ' + to + ' -y "' + croppedFileWithPath + '"';
  } else {
    ffmpegCommand = '"' + pathToFfmpeg + '" -loglevel ' + ffmppegLogLevel + ' -ss ' + start + ' -i "' + realUrl + '" -t ' + to + scale + ' -y "' + croppedFileWithPath + '"';
  }
  ...
  exec(ffmpegCommand, {
    ...
    if (Number(code) === 0) {
      ...
      var ffprobeCommand = '"' + pathToFfprobe + '" -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "' + croppedFileWithPath + '"';
...
var embedLogoAsWatermark = function(croppedFilename, extractor) {
  ...
  var filterComplex = '"[0:v][1:v] overlay=5:5:enable=\'between(t,0,20)\':format=rgb,format=yuv420p"';

  var ffmpegCommand = '"' + pathToFfmpeg + '" -loglevel ' + ffmppegLogLevel + ' -i "' + croppedFileWithPath + '" -i "' + imageFileWithPath + '" -filter_complex ' + filterComplex + ' -c:v libx264 -preset veryfast -crf 30 -y "' + finalcroppedFileWithPath + '"';
