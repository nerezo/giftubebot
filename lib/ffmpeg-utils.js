...
var cropVideoFromUrl = function(start, to, resolution, realUrl, videoId) {
  ...
  if (realUrl.indexOf('m3u8') > -1) {
    ffmpegCommand = 'ffmpeg -loglevel ' + ffmppegLogLevel + ' -i "' + realUrl + '" -c copy -ss ' + start + ' -t ' + to + ' -y -an "' + croppedFileWithPath + '"';
  } else {
    ffmpegCommand = 'ffmpeg -loglevel ' + ffmppegLogLevel + ' -ss ' + start + ' -i "' + realUrl + '" -t ' + to + scale + ' -y -an "' + croppedFileWithPath + '"';
  }
...
var embedLogoAsWatermark = function(croppedFilename, extractor) {
  ...
  var ffmpegCommand = 'ffmpeg -loglevel ' + ffmppegLogLevel + ' -i "' + croppedFileWithPath + '" -i "' + imageFileWithPath + '" -filter_complex ' + filterComplex + ' -c:v libx264 -preset veryfast -crf 30 -y -an "' + finalcroppedFileWithPath + '"';
