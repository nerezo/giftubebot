...
var cropVideoFromUrl = function(start, to, resolution, realUrl, videoId) {
  ...
  var croppedFilename = utils.sha256(start + '_' + to + '_' + videoId) + '_' + utils.defaults.cropFilePostfix + '.mp4';
  ...
  if (realUrl.indexOf('m3u8') > -1) {
    ffmpegCommand = 'ffmpeg -loglevel ' + ffmppegLogLevel + ' -i "' + realUrl + '" -c copy -ss ' + start + ' -t ' + to + ' -y "' + croppedFileWithPath + '"';
  } else {
    ffmpegCommand = 'ffmpeg -loglevel ' + ffmppegLogLevel + ' -ss ' + start + ' -i "' + realUrl + '" -t ' + to + scale + ' -y "' + croppedFileWithPath + '"';
  }
...
var embedLogoAsWatermark = function(croppedFilename, extractor) {
  ...
  var ffmpegCommand = 'ffmpeg -loglevel ' + ffmppegLogLevel + ' -i "' + croppedFileWithPath + '" -i "' + imageFileWithPath + '" -filter_complex ' + filterComplex + ' -c:v libx264 -preset veryfast -crf 30 -y "' + finalcroppedFileWithPath + '"';
