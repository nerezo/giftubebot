...
var getVideoInfo = function(url) {
  ...
  ytdl.getInfo(url, function(err, info) {
    if (err) {
      ...
      // Check if the extractor is different from youtube or vimeo. There are no duration value in their informations.
...
var getSuitableVideoFormat = function(url, resolutionLimit) {
  ...
  exec(command, {
    ...
    utils.logger.trace('youtube-dl list all formats command status:', code);
