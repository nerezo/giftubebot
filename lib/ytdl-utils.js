...
var getVideoInfo = function(url) {
  ...
  ytdl.getInfo(url, ['--format=bestvideo+bestaudio/best'], {maxBuffer: 5*1000*1024}, function(err, info) {
    if (err) {
...
var getSuitableVideoFormat = function(url, resolutionLimit) {
  ...
  var command = 'youtube-dl --list-formats --prefer-free-formats "' + url + '" |' + grepFilters + '|' + awkFilters + '|' + sortFilters;
...
var getRealVideoUrl = function(url, start, resolution) {
  ...
  var command = 'youtube-dl --get-url ' + format + ' "' + url + '"';
