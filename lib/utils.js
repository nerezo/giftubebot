...
var utils = function () {
    ...
    log4js.configure('log4js_configuration.json', {cwd: __dirname + '/../'});
    ...
    return {
        ...
        // Checks value of passed to parameter.
        validateToParam: function (to) {
            if (typeof to === 'undefined') {
                throw (new Error('to parameter is mandatory!'));
            }

            // If to value is not a number and wrong formatted duration string throwing an exception.
            if (isNaN(to) && (to.length !== 8 || (to[2] !== ':' && to[5] !== ':'))) {
                var message = 'to value should be in this format "hh:mm:ss" i.e. "00:12:35" or only a numeric value.';
