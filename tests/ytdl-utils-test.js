...
describe('ytdl-utils.js - youtubeInfo(<url>) unit tests', function () {

    it('youtubeInfo(<url>) should return information result within a promise', function (done) {
        ...
        ytdlUtils.youtubeInfo(url).done(function (result) {
    ...
    it('youtubeInfo(<url>) should return a proper error result within a promise', function (done) {
        var url = 'https://www.youtube.com/watch?v=kZPtOf6gs5g';
        var err = 'Video not found!';

        sinon.stub(ytdl, 'getInfo').yields(err, undefined);

        ytdlUtils.youtubeInfo(url).catch(function (result) {
...
describe('ytdl-utils.js - youtubeDownload(<file>, <format>, <url>) unit tests', function () {
    ...
    it('youtubeDownload(<file>, <format>, <url>) should return information result within a promise', function (done) {
        ...
        ytdlUtils.youtubeDownload(file, format, url).done(function (result) {
    ...
    it('youtubeDownload(<file>, <format>, <url>) should return a proper error result within a promise', function (done) {
        ...
        ytdlUtils.youtubeDownload(file, format, url).catch(function (result) {
            expect(ytdlUtils.ytdlWrapper.calledOnce).to.be.true;
            expect(videoMock.on.calledTwice).to.be.true;
            expect(fs.createWriteStream.calledOnce).to.be.true;
            expect(videoMock.pipe.calledOnce).to.be.true;
